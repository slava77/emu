...
void ConfigurationEditor::displayConfiguration(xgi::Output * out,const std::string &configName,const std::string &identifier) {
	try {
...
void ConfigurationEditor::displayDiff(std::ostream * out,const std::string &configName,const std::string &identifier,ChangeSummary &changes) {
	try {
...
bool ConfigurationEditor::displayCommonTableElements(std::ostream * out,const std::string &configName,const std::string &identifier,xdata::Table &currentTable,const std::string &display) {
...
void ConfigurationEditor::displayChildConfiguration(xgi::Output * out,const std::string &configName,const std::string &parentIdentifier) {
   	if (currentTables.count(configName)) {		
  		std::map<std::string,xdata::Table> &tables=currentTables[configName];
  		std::map<std::string,xdata::Table>::iterator table;
  ...
  //if (TStore_myEndcap_) {
  	...
  	if (currentTables[topLevelTableName_].size()) {
		...
		for (std::map<std::string,xdata::Table>::iterator tableDefinition=tableDefinitions.begin();tableDefinition!=tableDefinitions.end();++tableDefinition) {
			if (tableDefinition!=tableDefinitions.begin() && (*tableDefinition).first!=topLevelTableName_) *out << cgicc::br() << cgicc::hr() << cgicc::br();
...
void ConfigurationEditor::outputTableEditControls(xgi::Output * out,const std::string &tableName,const std::string &prefix) {
...
void ConfigurationEditor::outputSingleValue(std::ostream * out,xdata::Serializable *value,const std::string &column,const std::string &tableName,const std::string &identifier,int rowIndex) {
...
void ConfigurationEditor::outputTable(std::ostream * out,xdata::Table &results,const std::string &tableName,const std::string &identifier) {
	//this uses raw HTML tags because cgicc can't handle any nested tags, which makes it pretty much useless
	std::vector<std::string> columns=results.getColumns();
...
void ConfigurationEditor::outputEndcapSelector(xgi::Output * out) {
 ...
 void ConfigurationEditor::showTable(xgi::Input * in, xgi::Output * out ) 
    throw (xgi::exception::Exception) {
	    showHideTable(in,out,true);
    }
    
void ConfigurationEditor::hideTable(xgi::Input * in, xgi::Output * out ) 
    throw (xgi::exception::Exception) {
...
void ConfigurationEditor::viewValues(xgi::Input * in, xgi::Output * out ) 
    throw (xgi::exception::Exception) {
  	cgicc::Cgicc cgi(in);
  	std::string tableName=**cgi["table"];
...
void ConfigurationEditor::selectVersions(xgi::Input * in, xgi::Output * out ) 
    throw (xgi::exception::Exception) {
  	cgicc::Cgicc cgi(in);
...
void ConfigurationEditor::selectVersion(xgi::Input * in, xgi::Output * out ) 
    throw (xgi::exception::Exception) {
	try {
		cgicc::Cgicc cgi(in);
		std::string endcap_side=**cgi["side"];
...
void ConfigurationEditor::setValueFromString(xdata::Serializable *value,const std::string &newValue) throw (xdata::exception::Exception){
	std::string columnType=value->type();
/*
...
void ConfigurationEditor::setValue(xgi::Input * in, xgi::Output * out ) 
    throw (xgi::exception::Exception)
  {
  	cgicc::Cgicc cgi(in);
  	std::string tableName=**cgi["table"];
 ...
 void ConfigurationEditor::changeSingleValue(xgi::Input * in, xgi::Output * out ) 
    throw (xgi::exception::Exception) {
  	cgicc::Cgicc cgi(in);
  	std::string tableName=**cgi["table"];
 ...
 void ConfigurationEditor::getRangeOfTables(const cgicc::Cgicc &cgi,std::map<std::string,xdata::Table> &tables,std::map<std::string,xdata::Table>::iterator &firstTable,std::map<std::string,xdata::Table>::iterator &lastTable) {
  	std::string prefix=cgi("prefix");
	getRangeOfTables(prefix,tables,firstTable,lastTable);
}

void ConfigurationEditor::getRangeOfTables(const std::string &prefix,std::map<std::string,xdata::Table> &tables,std::map<std::string,xdata::Table>::iterator &firstTable,std::map<std::string,xdata::Table>::iterator &lastTable) {
	if (prefix.empty()) {
...
void ConfigurationEditor::incrementValue(xgi::Input * in, xgi::Output * out ) 
    throw (xgi::exception::Exception)
  {
  	cgicc::Cgicc cgi(in);
  	std::string tableName=**cgi["table"];
...
void ConfigurationEditor::getConfigIds(std::vector<std::string> &configIDs,const std::string endcap_side) throw (xcept::Exception) {
  xdata::Table results;
  std::string connectionID=connect();
...
void ConfigurationEditor::getDefinition(const std::string &connectionID, const std::string &insertViewName) throw (xcept::Exception) {
...
void ConfigurationEditor::insert(const std::string &connectionID, const std::string &insertViewName, xdata::Table &newRows) throw (xcept::Exception) {
