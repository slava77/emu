...
// calculate rate for proc
   if(flag_rate_5==1){
     ...
     if ( proc_rfirst_5==0 ){
       ...
       long int delta_jiffies = get_jiffies_64() - proc_previousjiffies_5;
       if ( delta_jiffies > HZ ){ // Update ~every second
	 proc_rate_5 = (double)( HZ * proc_rsum_5<<3 ) / delta_jiffies;
	 proc_previousjiffies_5 = get_jiffies_64();
	 proc_rsum_5 = 0;
       }
     }
   ...
   */
...
#ifdef DEBUG_PRINT
   // DEBUG START
   printk(KERN_INFO "ETH5 Length %d, flag_eevent %d, packet_count %04x\n",skb->len,flag_eevent_5,packet_count);
   ...
   if(flag_eevent_5==1){
     ...
     if ( skb->len+SKB_EXTRA == 64 ){
       ...
       printk(KERN_INFO "ETH5  minimum-length packet ends event\n");
#endif
     } // if ( skb->len+SKB_EXTRA == 64 )
     else{
