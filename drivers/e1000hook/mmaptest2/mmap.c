...
static int mmap_mmap(struct file *filp, struct vm_area_struct *vma)
{
        ...
        if (vma->vm_pgoff == 0) {
	  return do_rv_mmap(vma, (char *)vma->vm_start, 
...
static void __exit mmap_exit(void)
{
        ...
        cdev_del(&mmap_cdev);
        unregister_chrdev_region(mmap_dev, 1);
        rvunnail((char *)vmalloc_area,NPAGES*PAGE_SIZE);
        /* unreserve the pages */
        for (i = 0; i < NPAGES * PAGE_SIZE; i+= PAGE_SIZE) {
...
/* LSD rework of mmap handling for Kernel > 2.6.10 */
