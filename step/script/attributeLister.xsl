...
<xsl:transform 
  ...
  <xsl:template name="attributeXPath">    
    <xsl:for-each select="ancestor::*"><xsl:variable name="NAME" select="name()"/>/<xsl:value-of select="name()"/>[<xsl:value-of select="1+count(preceding-sibling::*[name()=$NAME])"/>]</xsl:for-each>/@<xsl:value-of select="name()"/><xsl:text>
</xsl:text>
  </xsl:template>
