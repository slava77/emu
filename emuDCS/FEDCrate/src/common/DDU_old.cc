...
void DDU::executeCommand(string command)
{
  enum DEVTYPE dv;
  // Program: Prog DDU
  if(command=="Load VMEPROM")vmeprgprom(filename);       
  if(command=="Load DDUPROM")dduprgprom(filename);       
  if(command=="Load INPROM")inprgprom(filename);        
  if(command=="Load Emergency VMEPROM")resetprgprom(filename);       
  if(command=="Dump userid/chipids")all_chip_info();    
  if(command=="VMEPROM idcode")printf(" %08x \n",vmeprom_idcode());
  if(command=="VMEPROM usercode")printf(" %08x \n",vmeprom_usercode());
  if(command=="DDUPROM idcode0")printf(" %08x \n",dduprom_idcode0());
  if(command=="DDUPROM idcode1")printf(" %08x \n",dduprom_idcode1());
  if(command=="DDUPROM usercode0")printf(" %08x \n",dduprom_usercode0());
  if(command=="DDUPROM usercode1")printf(" %08x \n",dduprom_usercode1());
  if(command=="DDUFPGA idcode")printf(" %08x \n",ddufpga_idcode());
  if(command=="DDUFPGA usercode")printf(" %08x \n",ddufpga_usercode());
  if(command=="INPROM idcode0")printf(" %08x \n",inprom_idcode0());
  if(command=="INPROM idcode1")printf(" %08x \n",inprom_idcode1());
  if(command=="INPROM usercode0")printf(" %08x \n",inprom_usercode0());
  if(command=="INPROM usercode1")printf(" %08x \n",inprom_usercode1());
  if(command=="INFPGA idcode0")printf(" %08x \n",infpga_idcode0());
  if(command=="INFPGA idcode1") printf(" %08x \n",infpga_idcode1());
  if(command=="INFPGA usercode0") printf(" %08x \n",infpga_usercode0());
  if(command=="INFPGA usercode1") printf(" %08x \n",infpga_usercode1());
  // DDU Control: ddufpga
  if(command=="DDUddu reset")ddu_reset();         
  if(command=="ddu low status")ddu_lowfpgastat();   
  if(command=="ddu high status")ddu_hifpgastat();  
  if(command=="ddu 32-bit status")ddu_fpgastat();     
  if(command=="Read FIFO-A Reg")ddu_checkFIFOa();     
  if(command=="Read FIFO-B Reg")ddu_checkFIFOb();     
  if(command=="Read FIFO-C Reg")ddu_checkFIFOc();     
  if(command=="FIFO Full Reg")ddu_rdfferr();
  if(command=="Read KillFiber Reg")ddu_rdkillfiber();
  if(command=="Load KillFiber Reg")ddu_loadkillfiber(hexval);    // needs to be fixed
  if(command=="CRC Error Reg")ddu_rdcrcerr();
  if(command=="Data Transmit Error Reg")ddu_rdxmiterr();
  if(command=="DMB Error Reg")ddu_rddmberr();
  if(command=="TMB Error Reg")ddu_rdtmberr(); 
  if(command=="ALCT Error Reg")ddu_rdalcterr();
  if(command=="LostInEvent Reg")ddu_rdlieerr();
  if(command=="DDU L1 Scale")ddu_rdscaler();
  if(command=="ERA Error Reg")ddu_rderareg();
  if(command=="ERB Error Reg")ddu_rderbreg();
  if(command=="ERC Error Reg")ddu_rdercreg();
  if(command=="Output Status")ddu_rdostat();
  if(command=="Load BX_Orbit")ddu_loadbxorbit(hexval);    // needs to be fixed
  if(command=="Read BX_Orbit")ddu_rdbxorbit();
  if(command=="Toggle DDU Cal L1A")ddu_lvl1onoff();
  if(command=="DDU shift test")ddu_shfttst(intval);
  if(command=="Read DDR VoteErr Count")ddu_rd_verr_cnt();
  if(command=="Read Consec Verr Count")ddu_rd_cons_cnt();
  if(command=="FIFO-0 Verr Count")ddu_fifo0verr_cnt();
  if(command=="FIFO-1 Verr Count")ddu_fifo1verr_cnt();
  if(command=="Early 200ns Verr Count")ddu_earlyVerr_cnt();
  if(command=="DDR Verr23 Count")ddu_verr23cnt();
  if(command=="DDR Verr55 Count")ddu_verr55cnt();
  if(command=="Read Board ID")ddu_rd_boardID();
  if(command=="DDU VME L1A")ddu_vmel1a();

  //DDU Control: infpga: infpga 0
  if(command=="infpga0 reset")infpga_reset(INFPGA0);
  if(command=="infpga0 Read L1 Scaler")infpga_rdscaler(INFPGA0);
  if(command=="infpga0 low status")infpga_lowstat(INFPGA0);
  if(command=="infpga0 high status")infpga_histat(INFPGA0);
  if(command=="infpga0 32-bit status")infpgastat(INFPGA0);
  if(command=="infpga0 shift test")infpga_shfttst(INFPGA0,intval);
  if(command=="infpga0 Check Fiber")infpga_CheckFiber(INFPGA0);
  if(command=="infpga0 DMB Sync")infpga_DMBsync(INFPGA0);
  if(command=="infpga0 FIFO Status")infpga_FIFOstatus(INFPGA0);
  if(command=="infpga0 FIFO Full")infpga_FIFOfull(INFPGA0);
  if(command=="infpga0 RxError")infpga_RxErr(INFPGA0);
  if(command=="infpga0 Timeout")infpga_Timeout(INFPGA0);
  if(command=="infpga0 XmitErr")infpga_XmitErr(INFPGA0);
  if(command=="infpga0 Lost Error")infpga_LostErr(INFPGA0);
  if(command=="infpga0 Stat-A Reg")infpga_StatA(INFPGA0);
  if(command=="infpga0 Stat-B Reg")infpga_StatB(INFPGA0);
  if(command=="infpga0 Stat-C Reg")infpga_StatC(INFPGA0);
  if(command=="infpga0 MemAvail")infpga_MemAvail(INFPGA0);
  if(command=="Fiber0 1-0 WrMemActive")infpga_WrMemActive(INFPGA0,0); // needs to be fixed
  if(command=="Fiber0 3-2 WrMemActive")infpga_WrMemActive(INFPGA0,0);
  if(command=="Fiber0 5-4 WrMemActive")infpga_WrMemActive(INFPGA0,0);
  if(command=="Fiber0 7-6 WrMemActive")infpga_WrMemActive(INFPGA0,0);
  if(command=="infpga0 Min MemAvail")infpga_Min_Mem(INFPGA0);
  //DDU Control: infpga: infpga 1
  if(command=="infpga1 reset")infpga_reset(INFPGA1);
  if(command=="infpga1 Read L1 Scaler")infpga_rdscaler(INFPGA1);
  if(command=="infpga1 low status")infpga_lowstat(INFPGA1);
  if(command=="infpga1 high status")infpga_histat(INFPGA1);
  if(command=="infpga1 32-bit status")infpgastat(INFPGA1);
  if(command=="infpga1 shift test")infpga_shfttst(INFPGA1,intval);
  if(command=="infpga1 Check Fiber")infpga_CheckFiber(INFPGA1);
  if(command=="infpga1 DMB Sync")infpga_DMBsync(INFPGA1);
  if(command=="infpga1 FIFO Status")infpga_FIFOstatus(INFPGA1);
  if(command=="infpga1 FIFO Full")infpga_FIFOfull(INFPGA1);
  if(command=="infpga1 RxError")infpga_RxErr(INFPGA1);
  if(command=="infpga1 Timeout")infpga_Timeout(INFPGA1);
  if(command=="infpga1 XmitErr")infpga_XmitErr(INFPGA1);
  if(command=="infpga1 Lost Error")infpga_LostErr(INFPGA1);
  if(command=="infpga1 Stat-A Reg")infpga_StatA(INFPGA1);
  if(command=="infpga1 Stat-B Reg")infpga_StatB(INFPGA1);
  if(command=="infpga1 Stat-C Reg")infpga_StatC(INFPGA1);
  if(command=="infpga1 MemAvail")infpga_MemAvail(INFPGA1);
  if(command=="Fiber1 1-0 WrMemActive")infpga_WrMemActive(INFPGA1,0); // needs to be fixed
  if(command=="Fiber1 3-2 WrMemActive")infpga_WrMemActive(INFPGA1,0);
  if(command=="Fiber1 5-4 WrMemActive")infpga_WrMemActive(INFPGA1,0);
  if(command=="Fiber1 7-6 WrMemActive")infpga_WrMemActive(INFPGA1,0);
  if(command=="infpga1 Min MemAvail")infpga_Min_Mem(INFPGA1);
  //DDU Control: vmeparallel
  if(command=="status")vmepara_status();
  if(command=="busy")vmepara_busy();
  if(command=="warning/near full")vmepara_fullwarn();
  if(command=="lost sync")vmepara_lostsync();
  if(command=="error")vmepara_error();
  if(command=="switches")vmepara_switch();
  if(command=="Rd Inreg0")vmepara_rd_inreg0();
  if(command=="Rd Inreg1")vmepara_rd_inreg1();
  if(command=="Rd Inreg2")vmepara_rd_inreg2();
  if(command=="Write Inreg")vmepara_wr_inreg(hexval);     // needs to be fixed
  //DDU Control: vmeserial
  if(command=="read status")read_status();
  if(command=="Wr Flash KillCh")write_page1();
  if(command=="Rd Flash KillCh")read_page1();
  if(command=="Wr Flash DDRthresh")write_page4();
  if(command=="Rd Flash DDRthresh")read_page4();
  if(command=="Wr Flash GBEthresh")write_page5();
  if(command=="Rd Flash GBEthresh")read_page5();
  if(command=="Wr Flash DDUid") write_page7();
  if(command=="Rd Flash DDUid")read_page7();
  if(command=="Rd DDR-FIFOthresh0")read_vmesd0();
  if(command=="Rd DDR-FIFOthresh1")read_vmesd1();
  if(command=="Rd DDR-FIFOthresh2")read_vmesd2();
  if(command=="Rd DDR-FIFOthresh3")read_vmesd3();
  if(command=="Force DDR Load")write_vmesdF();
  if(command=="DDU Temps")read_therm();
  if(command=="DDU Voltages")read_voltages();
}
