	...
	try {
		...
		if (fileData != cgi.getElements().end()) {
			...
			if (TextFile.is_open()) {
				...
				LOG4CPLUS_ERROR(getApplicationLogger(), error.str());
				XCEPT_DECLARE(emu::fed::FileException, e, error.str());
...
throw (emu::fed::ParseException)
{
	...
	try {
		FEDCrateParser parser(xmlFile_.toString().c_str());
	...
	try {
		...
		*out << "DCC Input/Output Rate Information" << cgicc::div() << std::endl;
		...
		for (int iFifo = 5; iFifo < 10; iFifo++) {
			ratesTable(iFifo+3,0) << "FIFO " << (iFifo+1) << " (DDU Slot " << myDCC->getDDUSlotFromFIFO(iFifo) << ")";
...
std::pair<unsigned int, emu::fed::FEDCrate *> emu::fed::EmuFCrateHyperDAQ::getCGICrate(cgicc::Cgicc cgi)
throw (emu::fed::OutOfBoundsException)
{
	...
	// Find the crate
	for (std::vector<FEDCrate *>::iterator iCrate = crateVector_.begin(); iCrate != crateVector_.end(); iCrate++) {
		if ((*iCrate)->number() == cgiCrate) return std::pair<unsigned int, FEDCrate *> (cgiCrate, (*iCrate));
	}
