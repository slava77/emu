/*****************************************************************************\
* $Id: DCC.cc,v 3.28 2009/01/29 15:31:23 paste Exp $
*
* $Log: DCC.cc,v $
...
throw (OutOfBoundsException, CAENException, DevTypeException)
{
	if (fifo > 11) XCEPT_RAISE(OutOfBoundsException, "there are only 12 FIFOs to check [0-11]");
	try {
...
throw (OutOfBoundsException, CAENException, DevTypeException)
{
	...
	if (dev == MPROM) {
		...
		XCEPT_RAISE(DevTypeException, "must supply a PROM device as an argument");
	}
