...
//-----------------------------------------------------------------------
// $Id: MPC.cc,v 2.6 2005/11/02 16:16:14 mey Exp $
// $Log: MPC.cc,v $
...
int MPC::read(int btd,char *buf_p,
    ...
    int xfer_len,
    int *xfer_done_p) {
  char *rcv = new char[xfer_len];
  unsigned short int *tdata;
  unsigned short int *aptr;
  int ncnt,i;
  theMode = READ;
  theController->start(this);
  aptr=(unsigned short int *)add;
  for(int i=0;i<xfer_len/2-1;i++){;
    tdata=(unsigned short int *)buf_p+i;
    vme_controller(0,aptr,tdata,rcv);
  }
   tdata=(unsigned short int *)buf_p+xfer_len/2-1;
   vme_controller(2,aptr,tdata,rcv);
   for(i=0;i<xfer_len;i++)buf_p[i]=rcv[i];
  theController->end();
  delete rcv;
  return ncnt;
...
void MPC::writeToAddress(unsigned long int add,
  ...
  aptr=(unsigned short int *)add;
    //printf(" address filled:  add %08x \n",*aptr);
  for(int i=0;i<xfer_len/2-1;i++){;
    tdata=(unsigned short int *)bufp+i;
    vme_controller(1,aptr,tdata,tmp);
  }
   tdata=(unsigned short int *)bufp+xfer_len/2-1;
   vme_controller(3,aptr,tdata,tmp);
}
