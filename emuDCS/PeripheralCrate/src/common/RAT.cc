...
void RAT::ReadRatIdCode(){
  ...
  rat_idcode_[0] = tmb_->bits_to_int(GetDRtdo(),GetRegLength(),0);
  //
  //
  ShfIR_ShfDR(ChipLocationRatProm,
	      PROMidCode,
	      RegSizeRatProm_PROMidCode);
  rat_idcode_[1] = tmb_->bits_to_int(GetDRtdo(),GetRegLength(),0);
  //
...
void RAT::ReadRatUserCode(){
  ...
  rat_usercode_[0] = tmb_->bits_to_int(GetDRtdo(),GetRegLength(),0);
  //
  //
  ShfIR_ShfDR(ChipLocationRatProm,
	      PROMuserCode,
	      RegSizeRatProm_PROMuserCode);
  rat_usercode_[1] = tmb_->bits_to_int(GetDRtdo(),GetRegLength(),0);
  //
