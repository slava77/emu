...
int main() {
  ...
  std::vector<Crate*> crateVector = myendcap->crates();
  //
  if (crateVector.size() > 1){
    std::cerr << "Error: only one PeripheralCrate allowed" << std::endl;
    exit(1);
  }
  std::vector<TMB*> tmbVector = myendcap->tmbs(crateVector[0]);
  if (tmbVector.size() > 1){
    std::cerr << "Error: only one TMB in xml file allowed" << std::endl;
    exit(1);
  }
  std::vector<DAQMB*> dmbVector = myendcap->daqmbs(crateVector[0]);
  if (dmbVector.size() > 1){
