...
//
void EmuPeripheralCrateConfig::TMBFiberReset(xgi::Input * in, xgi::Output * out )
  throw (xgi::exception::Exception) {
  //
  cgicc::Cgicc cgi(in);
  //
...
void EmuPeripheralCrateConfig::TMBUtils(xgi::Input * in, xgi::Output * out ) 
  ...
  *out << cgicc::form() << std::endl ;
  //
  std::string TMBFiberReset = toolbox::toString("/%s/TMBFiberReset",getApplicationDescriptor()->getURN().c_str());
  *out << cgicc::form().set("method","GET").set("action",TMBFiberReset);
  *out << cgicc::input().set("type","submit").set("value","Read");
  sprintf(buf,"%d",tmb);
  *out << cgicc::input().set("type","hidden").set("value",buf).set("name","tmb");
  *out << cgicc::input().set("type","hidden").set("value","read").set("name","mode");
  *out << cgicc::form() << std::endl;
  //
  *out << cgicc::table().set("border","1");
  *out << cgicc::tr();
