...
void XMLParser::ODAQMBParser(xercesc::DOMNode * pNode, Crate * theCrate, Chamber * theChamber)
{
  int slot = 0;
  ...
  if(slot == 0) {
    ...
    Crate *crate;
    crate=theCrate;
    ODAQMB * odaqmb_ = new ODAQMB(crate,theChamber,slot); 
    printf(" xxx out of ODAQMB \n");fflush(stdout);
    //
    int delay;
//     if ( fillInt("feb_dav_delay", delay)){
//       odaqmb_->SetFebDavDelay(delay);
//     }
//     if ( fillInt("tmb_dav_delay", delay) ){
//       odaqmb_->SetTmbDavDelay(delay);
//     }
//     if (fillInt("push_dav_delay", delay)){
//       odaqmb_->SetPushDavDelay(delay);
//     }
//     if(fillInt("l1acc_dav_delay", delay)){
//       odaqmb_->SetL1aDavDelay(delay);
//     }
//     if(fillInt("ALCT_dav_delay", delay)){
//       odaqmb_->SetAlctDavDelay(delay);
//     }
    if(fillInt("calibration_LCT_delay", delay)){
      odaqmb_->SetCalibrationLctDelay(delay);
    }
    if(fillInt("calibration_l1acc_delay", delay)){
      odaqmb_->SetCalibrationL1aDelay(delay);
    }
    if(fillInt("pulse_delay", delay)){
      odaqmb_->SetPulseDelay(delay);
    }
    if(fillInt("inject_delay", delay)){
      odaqmb_->SetInjectDelay(delay);
    }
    float value;
    if(fillFloat("pul_dac_set", value)){
      odaqmb_->SetPulseDac(value);
    }
    if(fillFloat("inj_dac_set", value)){
      odaqmb_->SetInjectorDac(value);
    }
    if(fillFloat("set_comp_thresh", value)){
      odaqmb_->SetCompThresh(value);
      for(int cfeb=0; cfeb<7; cfeb++) odaqmb_->SetCompThresholdsCfeb(cfeb,value);
    }
    int mode;
    if(fillInt("comp_mode", mode)){
      odaqmb_->SetCompMode(mode);
      for(int cfeb=0; cfeb<7; cfeb++) odaqmb_->SetCompModeCfeb(cfeb,mode);
    }
    if(fillInt("comp_timing", delay)){
      odaqmb_->SetCompTiming(delay);
      for(int cfeb=0; cfeb<7; cfeb++) odaqmb_->SetCompTimingCfeb(cfeb,delay);
    }
    if(fillInt("pre_block_end", delay)){
      odaqmb_->SetPreBlockEnd(delay);
      for(int cfeb=0; cfeb<7; cfeb++) odaqmb_->SetPreBlockEndCfeb(cfeb,delay);
    }
    if (fillInt("cfeb_cable_delay", delay)) {
      odaqmb_->SetCfebCableDelay(delay); }
    if (fillInt("tmb_lct_cable_delay", delay)) {
      odaqmb_->SetTmbLctCableDelay(delay); }
    if (fillInt("cfeb_dav_cable_delay", delay)) {
      odaqmb_->SetCfebDavCableDelay(delay); }
    if (fillInt("alct_dav_cable_delay", delay)) {
      odaqmb_->SetAlctDavCableDelay(delay); }

//    int id;
//    if(fillInt("crate_id",id)){
//      odaqmb_->SetCrateId(id);
//    }
      odaqmb_->SetCrateId(theCrate->CrateID());
    if ( fillInt("feb_clock_delay",delay)){
      odaqmb_->SetCfebClkDelay(delay);
    }
    if ( fillInt("xLatency",delay)){
      odaqmb_->SetxLatency(delay);
      for (int cfeb=0; cfeb<5; cfeb++) odaqmb_->SetL1aExtraCfeb(cfeb,delay);
    }
    if (fillInt("xFineLatency",delay)) {
      odaqmb_->SetxFineLatency(delay);
    }
    // just use the variable 'delay' for killinput
    if (fillInt("kill_input",delay)) {
      odaqmb_->SetKillInput(delay);
    }
    //
    if (fillInt("power_mask",delay)) {
      odaqmb_->SetPowerMask(delay);
    }
    //
    long int long_value;
    if (fillLongIntX("dmb_cntl_firmware_tag", long_value) ) 
      odaqmb_->SetExpectedControlFirmwareTag(long_value);
    //
    if (fillInt("dmb_vme_firmware_tag", delay) ) 
      odaqmb_->SetExpectedVMEFirmwareTag(delay);
    //
    ...
    while (daughterNode) {
      if (daughterNode->getNodeType() == xercesc::DOMNode::ELEMENT_NODE) {
	if (strcmp("DCFEB",xercesc::XMLString::transcode(daughterNode->getNodeName()))==0){
	  parseNode(daughterNode);
	  if(fillInt("cfeb_number", number)){
	    //odaqmb_->SendOutput("CFEB");
	    if ( number <7 ){
...
//fg
		...
		odaqmb_->SetExpectedCFEBFirmwareTag(number,long_value);
	      ...
	      fillIntX("kill_chip4",kill_chip[4]);
	      fillIntX("kill_chip5",kill_chip[5]);
              printf(" got kill data \n");fflush(stdout);
	      //
	      for(int chip=0;chip<6;chip++){
		//
		odaqmb_->SetKillChip(number,chip,kill_chip[chip]);
		//
		for(int chan=0;chan<16;chan++){
		  unsigned short int mask=(1<<chan);
		  if((mask&kill_chip[chip])==0x0000){
		    //odaqmb_->shift_array_[number][5-chip][chan]=NORM_RUN;
		    odaqmb_->SetShiftArray(number,7-chip,chan,NORM_RUN);
		  }else{
		    //odaqmb_->shift_array_[number][5-chip][chan]=KILL_CHAN;
		    odaqmb_->SetShiftArray(number,7-chip,chan,KILL_CHAN);
		  }
		}
	      } 	      
	      //
              printf(" about to pushback dcfeb \n");fflush(stdout);
	      odaqmb_->dcfebs_.push_back(dcfeb);
              printf(" dcfeb is pushed back \n");fflush(stdout); 
	    } else {
	      std::cout << "ERROR: CFEB range 0-4" <<std::endl;
	    }
	  }
...
void XMLParser::CSCParser(xercesc::DOMNode * pNode, Crate * theCrate, xercesc::DOMNode * pNodeGlobal)
{
  ...
  while (pNode1) {
	...
	DAQMBParser(pNode1, theCrate, csc_);
      ...
      //
      if (strcmp("ODAQMB",xercesc::XMLString::transcode(pNode1->getNodeName()))==0) {  
	ODAQMBParser(pNode1, theCrate, csc_);
      }
      //
