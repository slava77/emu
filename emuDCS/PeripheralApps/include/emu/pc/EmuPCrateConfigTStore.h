...
private:
	bool parseTestSummaryChamber(std::istringstream &testSummary, xdata::Table &changes,int &TTCrxID,std::string &chamberName) throw (xdaq::exception::Exception);
	...
	xdata::Table &getCachedDiff(const std::string &insertViewName,int crateID/*,xdata::UnsignedInteger64 &_vcc_config_id*//*,Crate *thisCrate*/) throw (xcept::Exception);
	xdata::Table &getCachedTable(const std::string &insertViewName,int crateID/*,xdata::UnsignedInteger64 &_vcc_config_id*//*,Crate *thisCrate*/) throw (xcept::Exception);
