...
int main(int argc, char **argv)
{
  ...
  slow_controls.push_back("hv");
  slow_controls.push_back("lv");
  slow_controls.push_back("temp");
  slow_controls.push_back("chip");
  slow_controls.push_back("mrtn");
  slow_controls.push_back("gas");
  slow_controls.push_back("pt100");
  slow_controls.push_back("wth");
  slow_controls.push_back("crb");
...
//==============================================================
  int found=0; 
        for(int i=0;i<slow_controls.size();i++){
          
	  if(slow_controls[i]==string(rc2dcs->slow_control)){
            found=1;
            break;
	  }
	  ////else printf("%s %s\n",slow_controls[i].c_str(),rc2dcs->slow_control);
	}
	if(!found){
           printf("ERROR: no such slow control: %s\n",slow_control.c_str());
           exit(0);
	}
