//-----------------------------------------------------------------------
// $Id: TestBeamCrateController_simple.cc,v 3.0 2006/01/18 17:52:27 geurts Exp $
// $Log: TestBeamCrateController_simple.cc,v $
...
//=================================================
void TestBeamCrateController::configure_simple() {
  // read the configuration
#ifdef USEDCS
  DcsSetup();   // DCS contrib
  if (mEmuDcs) DcsDisable(); // DCS contrib
...
void TestBeamCrateController::configure(Crate * crate) {
  ...
  MPC * mpc = crate->mpc();
  DDU * ddu = crate->ddu();
printf("ENTERING TestBeamCrateController::configure\n");
 ccb->configure();
  ::sleep(1);
printf("FINISHING CCB ************************************************************************\n");
//  ::sleep(2);
  std::vector<TMB*> myTmbs = theSelector.tmbs(crate);
  for(unsigned i =0; i < myTmbs.size(); ++i) {
        myTmbs[i]->configure();
  }
printf("FINISHING TMB************************************************************************\n");
  ::sleep(1);
  for(unsigned i =0; i < myTmbs.size(); ++i) {
...
//================================================================================
//=============================================================================================
void TestBeamCrateController::configure_simple(Crate * crate) {
  CCB * ccb = crate->ccb();
  MPC * mpc = crate->mpc();
  DDU * ddu = crate->ddu();
printf("ENTERING TestBeamCrateController::configure\n");
