...
// Stan: test if queue has been turned off
         if(netif_queue_stopped(dev))
                    printk(KERN_INFO "tx queue is stopped !\n");
...
rio_free_tx (struct net_device *dev, int irq) 
{
	...
	while (entry != np->cur_tx) {
		...
		if (irq)
			...
			dev_kfree_skb (skb);
			// printk(KERN_INFO "rio_free_tx %d %d \n",entry,np->cur_tx); 
...
receive_packet (struct net_device *dev)
{
	...
	/* If RFDDone, FrameStart and FrameEnd set, there is a new packet in. */
        // printk(KERN_INFO " incoming packet seen\n");
	while (1) {
		struct netdev_desc *desc = &np->rx_ring[entry];
		int pkt_len;
		u64 frame_status;
                u64 frame_status2; /* durkin kludge */
		...
		pci_dma_sync_single (np->pdev, desc->fraginfo, np->rx_buf_sz,
				     PCI_DMA_FROMDEVICE);
		/* durkin kludge - get rid of erroneous length errors */
                frame_status2=frame_status;   
		frame_status = frame_status & 0xffffffffffdfffff;
		// printk(KERN_INFO "frame_status %32x pkt_len %d \n",frame_status2,pkt_len);
		...
		if (frame_status & RFS_Errors) {
			...
			struct sk_buff *skb;
                        ...
                        /* durkin kludge - fix erroneous length errors */
                       if (frame_status2 & RFS_Errors) {
			np->stats.rx_packets= np->stats.rx_packets+1;
                        np->stats.rx_bytes=np->stats.rx_bytes+pkt_len;
		       }
