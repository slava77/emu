...
const unsigned short dduCrcTable[256] = {
...
void dduBinExaminer::crcALCT(bool enable){
	checkCrcALCT = enable;
	if( checkCrcALCT )
...
void dduBinExaminer::crcTMB(bool enable){
	checkCrcTMB = enable;
	if( checkCrcTMB )
...
void dduBinExaminer::crcCFEB(bool enable){
	checkCrcCFEB = enable;
	if( checkCrcCFEB )
...
void dduBinExaminer::crcDDU(bool enable){
        checkCrcDDU = enable;
        if( checkCrcDDU )
		sERROR[3] = " DDU CRC Error                                    ";
        else
                sERROR[3] = " DDU CRC Error ( disabled )                       ";
}

dduBinExaminer::dduBinExaminer(void):nERRORS(25),nWARNINGS(5),sERROR(nERRORS),sWARNING(nWARNINGS),sERROR_(nERRORS),sWARNING_(nWARNINGS){
...
// new old code for DDU CRC checks
		if( checkCrcDDU && fDDU_Header ){
			const unsigned char *block = (const unsigned char *)buf0;
			for(int i=0; i<8; i++) DDU_CRC = dduCrcTable[((DDU_CRC>>8) ^ block[i]) & 0xFF] ^ (DDU_CRC << 8);
		}
//end
