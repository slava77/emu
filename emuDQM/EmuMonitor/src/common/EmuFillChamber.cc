...
//	Efficiency of the chamber
	...
	if(fill_histo) {
		...
		hname = Form("hist/h%d_CSC_Rate", ChamberID);
...
//		Take total number of events correcponding to this particular DMB
		...
		hname = Form("hist/h%d_CSC_Efficiency", ChamberID);
...
//KK
 		if(check_bin_error){
			...
			while( chamber != checkerErrors.end() ){
				hname = Form("hist/h%dBinCheck_ErrorStat_Table", chamber->first);
			...
			while( chamber != checkerWarnings.end() ){
				hname = Form("hist/h%dBinCheck_WarningStat_Table", chamber->first);
...
//	Unpacking CFEB information from DMB header
	...
	hname = Form("hist/h%dDMB_CFEB_Active", ChamberID);
	//h[hname]->Fill((dmbTrailer.header_1a>>5)&0x1F); //KK
	h[hname]->Fill(dmbHeader.cfebActive()); //KK

	hname = Form("hist/h%dDMB_CFEB_Active_vs_DAV", ChamberID);
...
/*
		...
		for (int nLayer=1; nLayer<=6; nLayer++) {
			...
			vector<CSCComparatorOutput> compOutData = clctData.comparatorOutputs(nLayer);
			for (vector<CSCComparatorOutput>:: iterator compOutDataItr = compOutData.begin(); compOutDataItr != compOutData.end(); ++compOutDataItr) {
				int hstrip = 2*compOutDataItr->strip() + compOutDataItr->halfStrip();
				int tbin_clct = (int)compOutDataItr->time();
				if(CheckLayerCLCT) {
...
//-------------E
			if(DebugCFEB) {
				...
				cout << "Chamber ID = "<< ChamberID << " Crate ID = "<< crateID << " DMB ID = " << dmbID << endl;
...
// Fill Histogram with number of unpacked datas
	...
	hname = Form("hist/h%dDMB_FEB_DAV", ChamberID);
	h[hname]->Fill(FEBdav);

	hname = Form("hist/h%dDMB_FEB_unpacked_vs_DAV", ChamberID);
