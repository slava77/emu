...
//	Check DDU Readout Error Status
	...
	if(errorStat != 0) {
		if(error_printout) {
			...
			// "D**EmuFillCommon> event #" << dec << nEvents <<
			"Nonzero DDU Readout Error Status is observed: 0x" << hex << errorStat);
        	}
		if(fill_histo) {
			if(error_printout) LOG4CPLUS_WARN(logger_, 
			// "E**EmuFillCommon> event #" << dec << nEvents <<
			"Filling histogram of DDU Readout Error Status");
		...
		if(debug_printout) LOG4CPLUS_INFO(logger_, 
			// "D**EmuFillCommon> event #" << dec << nEvents << 
			"DDU Readout Error Status is OK: 0x" << hex << errorStat);
	}
...
//KK end
		...
		if(BinaryErrorStatus != 0) {
			if(error_printout) {
				LOG4CPLUS_WARN(logger_,
			// "E**EmuFillCommon> event #" << dec << nEvents << 
			"Nonzero Binary ERROR Status is observed: 0x" << hex << BinaryErrorStatus);
        		}
			if(fill_histo) {
				if(error_printout) LOG4CPLUS_DEBUG(logger_, 
					// "E**EmuFillCommon> event #" << dec << nEvents <<
			...
			if(debug_printout) LOG4CPLUS_INFO(logger_,
				// "D**EmuFillCommon> event #" << dec << nEvents << 
				"DDU Binary Error Status is OK: 0x" << hex << BinaryErrorStatus);
		}
		if(BinaryWarningStatus != 0) {
			if(error_printout) {
				LOG4CPLUS_WARN(logger_, 
				// "W**EmuFillCommon> event #" << dec << nEvents << 
				"Nonzero Binary Warning Status is observed: 0x" << hex << BinaryWarningStatus);
        		}
			if(fill_histo) {
				if(error_printout)  LOG4CPLUS_DEBUG(logger_,
				// "W**EmuFillCommon> event #" << dec << nEvents <<
			...
			if(debug_printout) LOG4CPLUS_INFO(logger_, 
			// "D**EmuFillCommon> event #" << dec <<nEvents <<
			"DDU Binary Warning Status is OK: 0x" << hex << BinaryWarningStatus);
		}
	}
	else if(debug_printout) LOG4CPLUS_INFO(logger_, 
		// "D**EmuFillCommon> event #" << dec << nEvents <<
		"Binary checking skipped");
...
//	Accept or deny event according to DDU Readout Error and dduCheckMask
	if (((unsigned long)errorStat & dduCheckMask) > 0) {
		if (error_printout) LOG4CPLUS_WARN(logger_, 
			// "E**EmuFillCommon> event #" << dec << nEvents <<
			"Event skiped because of DDU Readout Error");
		EventDenied = true;
	}
