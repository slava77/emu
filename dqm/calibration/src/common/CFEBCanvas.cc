#include "emu/dqm/calibration/CFEBCanvas.h"

CFEBCanvas::CFEBCanvas (const char* name, const char* title, Int_t Nbinsx, Double_t xlow, Double_t xup, Int_t Nbinsy, Double_t ylow, Double_t yup)
{
// CFEBCanvas::CFEBCanvas (Int_t Nbinsx, Double_t xlow, Double_t xup, Int_t Nbinsy, Double_t ylow, Double_t yup) {
...
// Lines to show performance range on right histogram
  ...
  theMainCanvas = new TCanvas(name, title,DEF_WIDTH,DEF_HEIGHT);
  ...
  theRightHisto = new TH1F((id+"_theRightHistogram").c_str(), "theRightHistogram", theNbinsy, theYlow, theYup);
  ...
  theLeftHistoBackground = new TH2F((id+"_background").c_str(), "background", theNbinsx, theXlow, theXup, theNbinsy, theYlow, theYup);
  ...
  for (fIndexLeftHisto = 0; fIndexLeftHisto < NLAYERS; fIndexLeftHisto++)
    {
      std::string hname = Form("%s_Layer_%d",id.c_str(),fIndexLeftHisto + 1);
      fTitleLeftHisto = Form("Layer_%d",fIndexLeftHisto + 1);
      theLeftHisto[fIndexLeftHisto] = new TH2F(hname.c_str(), fTitleLeftHisto.c_str(), theNbinsx, theXlow, theXup, theNbinsy, theYlow, theYup);
  ...
  float fMarkerSize = 0.5; ///< Marker size
...
void CFEBCanvas::SetTitle (std::string text)
{

  theTitlePad->GetLine(0)->SetText(0.0, 0.0, text.c_str());

}

void CFEBCanvas::SetXTitle (std::string text)
{
...
void CFEBCanvas::SetYTitle (std::string text)
{
...
void CFEBCanvas::AddTextTest(std::string text)
{
...
void CFEBCanvas::AddTextResult(std::string text)
{
...
void CFEBCanvas::AddTextDatafile (std::string text)
{
...
void CFEBCanvas::AddTextRun (std::string text)
{
...
void CFEBCanvas::AddTextAnalysis (std::string text)
{
...
void CFEBCanvas::AddTextEntries (std::string text)
{
...
void CFEBCanvas::AddTextLimits (std::string text)
{
...
void CFEBCanvas::SetLimits (Double_t lowLowLimit,Double_t lowLimit,Double_t highLimit,Double_t highHighLimit)
{
...
int CFEBCanvas::Fill (CFEBData& data, CFEBData& mask)
{
...
// Add information (Entries, Mean, RMS, Underflow, Overflow) to StatPad of right histogram
  ...
  if (!fIsRedSolid && !fIsYellowSolid && fIsEmpty)
    {
      pFillColor = theColorBlueLight;
      fQualityTest = 3;
    }
  if (fIsRedSolid)
    {
      pFillColor = theColorRedLight;
      fQualityTest = 4;
    }

  theMainCanvas->SetFillColor(pFillColor);

  return fQualityTest;
}

void CFEBCanvas::Draw (void)
{
...
void CFEBCanvas::SetCanvasSize(uint32_t w, uint32_t h)
{
  if (theMainCanvas) theMainCanvas->SetCanvasSize(w,h);
}

void CFEBCanvas::SaveAs (void)
{
  theMainCanvas->SaveAs("canvas_6gr1h.png");
  theMainCanvas->SaveAs("canvas_6gr1h.ps");
}

void CFEBCanvas::SaveAs (std::string name)
{
  if (theMainCanvas) theMainCanvas->SaveAs((name +".png").c_str());
}

void CFEBCanvas::Write ()
{
  if (theMainCanvas) theMainCanvas->Write();
}
