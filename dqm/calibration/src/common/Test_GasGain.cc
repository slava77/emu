...
void Test_GasGain::initCSC(std::string cscID)
{
  ...
  const unsigned int N_Samples=16;
  ...
  for (unsigned int i_layer=0;i_layer<NLayer;i_layer++){
    for (unsigned int i_cfeb=0;i_cfeb<N_CFEBS;i_cfeb++){
      for (unsigned int i_segment=0;i_segment<nsegments;i_segment++){
        ...
        int test= sprintf(histo_name_st,"hist_%d_%d_%d",i_layer,i_cfeb,i_segment);
        char title[50];
        int test_title=sprintf(title, "Layer %d, HV segment %d cfeb %d",i_layer+1,i_segment+1,i_cfeb+1);
        const char* hist_title=title;
        const char* histo_name=histo_name_st;
        unsigned int index=i_segment+i_cfeb*nsegments+i_layer*N_CFEBS*nsegments;
    ...
    for(unsigned int i_strip_bin=0; i_strip_bin<Nbins_strips;i_strip_bin++){
      for(unsigned int i_wire_bin=0; i_wire_bin<Nbins_wires;i_wire_bin++){
        char histo_name_st[50];
        int test= sprintf(histo_name_st,"hist_%d_%d_%d",i_layer,i_strip_bin,i_wire_bin);
        char title[50];
        int test_title=sprintf(title, "Layer %d, strip bin %d, wire bin %d",i_layer+1,i_strip_bin+1,i_wire_bin+1);
        const char* hist_title=title;
        const char* histo_name=histo_name_st;
        unsigned int index=i_layer*Nbins_wires*Nbins_strips+i_strip_bin*Nbins_wires+i_wire_bin;
	...
	if(alctHeader && alctTrailer){
	  ...
	  if(alctData){
		...
		for(unsigned int istrip=1;istrip<=NStrips_per_CFEB;istrip++){
		  ...
		  if(tbin_max[icfeb*NStrips_per_CFEB+istrip-1]>(nTimeSamples-3)){
		  ...
		  for(unsigned int tbin=(tbin_pulse-2);tbin<(tbin_pulse+3);tbin++){
