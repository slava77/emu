...
xoap::MessageReference EmuMonitor::syncToCurrentRun(xoap::MessageReference node) throw (xoap::exception::Exception)
{
  appBSem_.take();
//  LOG4CPLUS_DEBUG (logger_, "Received sync to Current Run request");
  try
    {

      xoap::SOAPBody rb = node->getSOAPPart().getEnvelope().getBody();
      xdata::UnsignedInteger runNum = runNumber_;
      xdata::UnsignedInteger runStart = runStartUTC_;

      xoap::MessageReference msg = xoap::createMessage();
      xoap::SOAPEnvelope envelope = msg->getSOAPPart().getEnvelope();
      xoap::SOAPBody body = envelope.getBody();
      xoap::SOAPName commandName = envelope.createName("syncToCurrentRun","xdaq", "urn:xdaq-soap:3.0");
      xoap::SOAPElement command = body.addBodyElement(commandName );
      xdata::Integer localTid(i2o::utils::getAddressMap()->getTid(this->getApplicationDescriptor()));
      ...
      else
        {
          ...
          for (std::vector<xoap::SOAPElement>::iterator n_itr = content.begin();
               n_itr != content.end(); ++n_itr)
            {
              ...
              xoap::SOAPName runNumberStampTag ("runNumber", "", "");
              std::vector<xoap::SOAPElement> runNumberStampElement = n_itr->getChildElements (runNumberStampTag );
              for (std::vector<xoap::SOAPElement>::iterator f_itr = runNumberStampElement.begin();
                   f_itr != runNumberStampElement.end(); ++f_itr)
                {
                  runNum = f_itr->getValue();
                  // LOG4CPLUS_INFO(logger_, "Received runNumber " << runNum);
                }

              xoap::SOAPName runStartStampTag ("runStartUTC", "", "");
              std::vector<xoap::SOAPElement> runStartStampElement = n_itr->getChildElements (runStartStampTag );
              for (std::vector<xoap::SOAPElement>::iterator f_itr = runStartStampElement.begin();
