#include "emu/daq/reader/SLink.h"
...
emu::daq::reader::SLink::SLink( int boardIndex,  int format, bool debug )
  : emu::daq::reader::Base( format, debug ),
...
emu::daq::reader::SLink::~SLink(){
  close();
}

void emu::daq::reader::SLink::open() {
...
void emu::daq::reader::SLink::close() {
  fedkit_close (receiver);
}


int emu::daq::reader::SLink::reset(void){
  fedkit_close(receiver);
  sleep(1);
  receiver = fedkit_open(0, NULL);
  if (receiver == NULL) {
    std::cout << "emu::daq::reader::SLink::reset: FAILED TO INITIALIZE. Aborting ..." << std::endl;
    abort();
  }
  fedkit_start (receiver);
  return 0;
...
int emu::daq::reader::SLink::chunkSize() {
  //int cnt[2];
...
int emu::daq::reader::SLink::readDDU(unsigned short **buf) {
  // TODO
  return -1;
}
